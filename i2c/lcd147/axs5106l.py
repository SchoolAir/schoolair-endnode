import time
from smbus2 import SMBus, i2c_msg
from gpiozero import *
import RPi.GPIO   


AXS5106L_ADDRESS = 0x63

AXS5106L_LCD_TOUCH_MAX_POINTS = 2

TP_INT   = 4
TP_RST   = 17

    


class axs5106l():
    def __init__(self):
        self.GPIO = RPi.GPIO
        self.GPIO.setmode(self.GPIO.BCM)
        self.GPIO.setwarnings(False)
        # #Initialize I2C
        self.I2C = SMBus(1)
        # self.GPIO.setup(TP_INT, self.GPIO.IN,self.GPIO.PUD_UP)
        self.GPIO.setup(TP_RST, self.GPIO.OUT)
        # self.GPIO.add_event_detect(TP_INT,self.GPIO.FALLING,self.Int_Callback,5) 
        # pass
        self.GPIO_TP_INT = Button(TP_INT)                                                  # 使用GPIO Zero库中的Button类
        # self.GPIO_TP_RST = DigitalOutputDevice(TP_RST,active_high = True,initial_value =True)   # 使用GPIO Zero库中的DigitalOutputDevice类
        # self.GPIO_TP_INT.when_pressed = self.Int_Callback  # 中断函数
        
        self.coordinates = [{"x": 0, "y": 0} for _ in range(AXS5106L_LCD_TOUCH_MAX_POINTS)]
        self.point_count = 0
        self.touch_rst()
    
    def Int_Callback(self):
        self.read_touch_data()
    
    #Reset  复位    
    def touch_rst(self):
        self.GPIO.output(TP_RST, 0)  
        time.sleep(50 / 1000.0)
        self.GPIO.output(TP_RST, 1)  
        time.sleep(200 / 1000.0)
        
        
    def write_cmd(self, cmd):
        self.I2C.write_byte(AXS5106L_ADDRESS, cmd)

    def read_bytes(self, reg_addr, length):
        self.I2C.write_byte(AXS5106L_ADDRESS, reg_addr)
        read_msg = i2c_msg.read(AXS5106L_ADDRESS,length)
        self.I2C.i2c_rdwr(read_msg)
        data = list(read_msg)
        return data
    
    def read_touch_data(self):
        TOUCH_DATA_REG = 0x01
        
        buf = self.read_bytes(TOUCH_DATA_REG, 14)
        if buf is not None and buf[1]!= 0:
            self.point_count = buf[1]
            for i in range(2):
                self.coordinates[i]["x"] = 0
                self.coordinates[i]["y"] = 0
            for i in range(self.point_count):
                self.coordinates[i]["x"] = ((buf[2 + i * 6] & 0x0f) << 8) + (buf[3 + i * 6])
                self.coordinates[i]["y"] = ((buf[4 + i * 6] & 0x0f) << 8) + (buf[5 + i * 6])
    
    def get_touch_xy(self):
        point = self.point_count
        # 将触摸点数重置为 0
        self.point_count = 0

        if point != 0:
            # 返回触摸状态和坐标
            return point, self.coordinates
        else:
            # 返回 0 和空坐标列表
            return 0 , []
    
    
        